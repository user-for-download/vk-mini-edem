...
- **ConfirmProvider queue (2026-09-07):** double-confirm (profile deletion) was deterministically broken — second `confirm()` ran while the first Alert was still closing, `isOpenRef` guard returned `false`, silent exit. Provider now serializes via `chainConfirmTask` (`helpers/confirmQueue.ts`) and resolves only in `onClosed` (choice latched in action handler). Single confirms unchanged. Tests: `confirmQueue.test.ts` (3).
