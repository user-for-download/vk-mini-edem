...
- **Trip secondary actions inline (2026-09-07):** Share/Report live in a «Дополнительно» section at the bottom of `TripDetailsPanel` (new `TripSecondaryActions`, `ButtonGroup vertical` + 2× secondary buttons). Header kebab (`TripActionsSheet`) removed — VK client system buttons occupy the top-right corner and overlapped it. Handlers unchanged. Report button gated to participants (`isOwnTrip || myBooking`, mirrors `canSeePrivateDetails`) — strangers got `403 FORBIDDEN` from `canReport`; `ReportModal` maps 403 to «Жалоба недоступна». Entry button reflects sent state (`alreadyReported` from `GET /reports`): «Жалоба уже отправлена», disabled — modal never opens. Tests: `TripActionsMenu.test.tsx` (5 SSR).
