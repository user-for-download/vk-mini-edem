...
Updated: 2026-09-10 (VK removal, tg-migration-24–26)

## Project

Edem is a Telegram Mini App for shared rides. It is an npm-workspaces TypeScript monorepo:

- `telegram-app/`: React 19, telegram-ui, Vite, TanStack Query, Zustand, Telegram SDK (`@telegram-apps/sdk-react`).
...
- `e2e/`: Telegram parity Playwright suite (`telegram-parity.mjs`, 16 steps, driver→passenger flow).

VK Mini App removed 2026-09-10 (tg-migration-25/26, owner GO + empty-prod exception; full copy preserved in a separate repository). VK history below is archive.

## Runtime

- Local root `npm run dev` builds `packages/contracts` first, then starts Telegram frontend on `3012` and backend on `3011`.
...
- Production Docker backend listens on `3000` and serves `telegram-app/dist` for `TELEGRAM_HOSTS` (other hosts → 404); the published bind address is configurable via `BACKEND_BIND_ADDR` in the root `.env` (default `127.0.0.1` for a same-host proxy, `0.0.0.0` for an external proxy plus firewall rules), while PostgreSQL is available only inside the Docker network.
- Production requires `DATABASE_URL`, `JWT_SECRET`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_HOSTS`, and `CORS_ORIGINS`.
- `ALLOW_DEV_AUTH` is disabled in production and only supports local/test mock auth.
- Node >= 22 is required (`engines` in root `package.json`; CI and the Docker image both run Node 22). Installs go through **npm** (`npm ci`, lockfile `package-lock.json`).
...
- Telegram initData is sent raw and unchanged (`retrieveRawInitData()`).
- Backend verifies the HMAC-SHA256 signature against `TELEGRAM_BOT_TOKEN` and rejects initData older than `TG_INIT_DATA_TTL_SECONDS` (default 3600); dev-bypass `hash=dev-hash` works only outside production with `ALLOW_DEV_AUTH`. Unconfigured route → 503.
- Display data comes from the signed initData `user` object but is sanitized; avatar accepts only https from the `t.me`/`telegram.org` allowlist and re-syncs on every login (not user-editable via API); name only replaces the `Пользователь Telegram <id>` placeholder, manually edited names (PATCH /users/me) are never overwritten. Identity and isVerified come from the verified `telegramUserId`.
...
- Auth rate limits are configurable: `TG_AUTH_RATE_WINDOW_MS`/`TG_AUTH_RATE_MAX` (default 5/5min) and `REFRESH_RATE_WINDOW_MS`/`REFRESH_RATE_MAX` (default 10/10min).
- Telegram-authed = verified: `isVerified` defaults to `true` and `verifiedAt` is set on the `/auth/telegram` upsert — the verified initData signature IS the verification.
...
- Feedback appeal for banned users: `POST /api/v1/feedback/appeal` — публичный (без токена, забаненному токен не выдаётся): тело `{ initData, subject, text }` (initData ≤ 4096), личность подтверждается подписью Telegram initData через `verifyTelegramInitData` (dev-hash при `ALLOW_DEV_AUTH` работает) — токены НЕ выдаются. Невалидная подпись → 401, пользователь не найден → 404, невалидное тело → 400 VALIDATION_FAILED; отдельный rate-limiter 5/час на IP (429). Обращение привязывается к `userId` — видно в админке (`GET /admin/feedback`). Mini-app: `useAuthStore` хранит initData при бане; `useCreateFeedbackMutation` маршрутизирует: есть токен → `POST /feedback`, нет → appeal (нет токена и нет initData → ошибка-снэкбар без краша).
...
- Banned users (`bannedAt` set) are rejected with 403 on every authenticated endpoint; ban is enforced at login (`/auth/telegram` → 403 with `banReason`, no tokens, active refresh tokens revoked), on `/auth/refresh` (403 with `banReason` + token revocation), `optionalAuth` (banned = guest), and WS auth (close 4403).
...
## Telegram Integration (VK removed 2026-09-10, tg-migration-25/26)

- Telegram SDK init follows the reactjs-template patterns on current scope (`@telegram-apps/sdk-react` 3.3.x); dev outside Telegram uses `mockEnv` (`hash='dev-hash'` dev-bypass + `signature='dev-signature'`).
- Session state reacts to browser visibility and WebView hide/restore events.
...
- WebSocket contract (`packages/contracts`) matches the implementation: server sends `auth:ok`, `ping` (keep-alive), and business events; client sends only `auth` and `pong`.
- WebSocket cleanup guards against stale sockets and reconnects after provider unmount; backoff 1s→30s+jitter, single socket, 4403 terminal, refresh single-flight, pause in background, resync.
- Onboarding/consent screens are Telegram-native (`Onboarding` component): agreement + privacy, 14+, accept/decline/delete. Done flag is backend-side: `User.onboardingVersion String?` — the client compares `user.onboardingVersion` with its own `ONBOARDING_VERSION`; bumping the constant re-shows the screens to all users once. Completion is `POST /api/v1/users/me/onboarding` (body `{version}`, trimmed string 1..50 chars); admins can reset the flag (`PATCH /api/v1/admin/users/:id/onboarding-reset` → null).
- Notifications: persisted in-app inbox + WebSocket hint; Bot API background delivery blocked by product decision. Deep-links validated against the TG route allowlist.
...
## Participant contact & push (VK removed 2026-09-10)

- VK DM button and VK push removed with the VK platform (tg-migration-25/26).
  Participant contact mechanism is an open product decision (privacy-safe,
  no Telegram username assumption). Background delivery via Bot API is blocked
  pending product approval (ADR `telegram-notification-delivery`).
- Current delivery: persisted in-app inbox + WebSocket hint; critical events
  (`booking_status_changed`, `trip_cancelled`, `trip_status_changed`) persist
  regardless of the optional toggle; duplicates deduped; deep-links validated
  against the TG route allowlist (`backend/src/services/telegramNotifications.ts`).
...
- **Telegram transition, Phase 1 — telegram-app foundation (2026-09-09):** workspace `telegram-app/` (React 19 + Vite 8 + `@telegram-apps/sdk-react@3.3.9` + `@telegram-apps/telegram-ui@2.1.13`; telegram-ui peer react ^18 — React 19 проверен SSR-рендером, defaultProps=0, bun peers не форсит; при рантайм-проблемах план Б — react@18 nested). Портированы: `api/client.ts` 1:1, `store/useAuthStore.ts` со сменой payload на `retrieveRawInitData()` (RAW-passthrough, статус-машина идентична), QueryClient-конфиг. UI-шелл: AppRoot (platform: ios→ios, остальное→base) + AuthGate-эквивалент на telegram-ui + заглушка главной. SDK init по паттернам reactjs-template на актуальном скоупе; mockEnv с `hash='dev-hash'` (dev-bypass Фазы 0) + `signature='dev-signature'` (схема launch params 3.3.x требует signature). **Отличия SDK 3.3.x от шаблона 3.0.8:** miniApp.mount() внутри монтирует themeParams (двойной mount → "already mounting" — явный убран); mount асинхронен (await перед bindCssVars — "component is unmounted"); onEvent — кортеж [method,payload]; AppRoot platform только base|ios; retrieveRawInitData → string|undefined. **Раздача прод-статики — host-based** (app.ts): Host из `TELEGRAM_HOSTS` (tg-edem.binetc.fun) → telegram-app/dist, иначе mini-app/dist; BotFather не трогали (кнопка уже на root). Docker: build telegram-app + COPY dist; compose прокидывает TELEGRAM_HOSTS; root `.env`: TELEGRAM_HOSTS=tg-edem.binetc.fun. Root scripts: `dev:tg` (:3012, proxy /api→:3011), `build:tg`. Dev-флоу: реальный токен убран из backend/.env (иначе отключает dev-bypass; для реальной валидации локально — shell-export TELEGRAM_BOT_TOKEN). Тесты: store 7/7 (RAW-passthrough assert, ban/deleted/unauth). Проверено: Playwright E2E в браузере (mock → dev-bypass → «Dev Telegram», 0 JS-ошибок), docker host-routing по ассетам (Db96N9ef=tg vs CRqNeKdd=vk), публичный домен отдаёт TG-сборку. Real-device check ✅ (2026-09-09): кнопка @edem_mini_bot открывает приложение, авторизация проходит — видны имя и Telegram ID. Осталось: проверка на реальном телефоне через кнопку бота.

- **Telegram transition, Phase 0 — auth (2026-09-09):** VK заморожен (tag `vk-final`; снапшот-репо `user-for-download/vk-mini-app-edem` получил полную историю git-push'ем). Telegram-auth внедрён на существующем бэкенде: Prisma `User.telegramUserId BigInt? @unique` (миграция `20260909120000_add_telegram_user_id`, BigInt — TG id > Int32; test-БД через `db:test:push` — миграции на ней не развёртываются, P3005); `POST /api/v1/auth/telegram` в `authRouter` (порядок проверок зеркален /vk: sanitize → zod → валидация → tombstone → upsert + P2002-ретрай → бан → токены; пустой `TELEGRAM_BOT_TOKEN` без dev-режима = 503 «not configured», как ADMIN_TOKEN); валидация — `@telegram-apps/init-data-node@2.0.10` (`validate(initData, token, {expiresIn: TG_INIT_DATA_TTL_SECONDS=3600})`, replay-кэш НЕ нужен — initData законно живёт в клиенте всю сессию, в отличие от одноразового VK-окна). **Нюансы пакета:** runtime `parse()` возвращает snake_case, хотя .d.ts обещает camelCase (каст через собственный `RawParsedUser`); у user-схемы пакета `first_name` обязателен — `parse()` бросает на подписанных данных без него → try/catch fail-closed (401, не 500); типизация через структурную обёртку `{user?: unknown}`, не `ReturnType<typeof parse>` (генерики дают разные типы для вызова и ReturnType). Dev-bypass в тестах: `hash=dev-hash` точное совпадение (никаких includes), без `TELEGRAM_BOT_TOKEN` при `ALLOW_DEV_AUTH`; реальная HMAC-ветка покрыта юнит-тестами через `sign()` из пакета. Профиль: display из подписанного user-объекта, но санитизация имени + аватар только `https` + host-allowlist `t.me`/`telegram.org` (defense in depth, зеркально VK); placeholder «Пользователь Telegram <id>», ручные имена не перезаписываются, аватар ресинхронизируется при входе. Contracts: `telegramAuthRequestSchema` (RAW initData 1..4096, пересортировка запрещена — HMAC не сойдётся). Env: `TELEGRAM_BOT_TOKEN` (опционален, НЕ secretEnv), `TG_INIT_DATA_TTL_SECONDS`, `TG_AUTH_RATE_*` (зеркало VK-лимитов). Тесты: `tests/unit/telegramSign.test.ts` (12), `tests/integration/telegram-auth.test.ts` (12, BigInt-диапазон 9_900_000+ не пересекается с VK-диапазонами), contracts +6. Backend 465/465, все typecheck зелёные. `telegramUserId` в API-ответы НЕ отдаётся (privacy, как vkUserId — t.me-ссылки будут в Фазе 2+). Skill `telegram-mini-app` установлен в `.opencode/skills/` (адаптирован: актуальный скоуп `@telegram-apps/*`, TON игнорируется).

- **Car plate optional (2026-09-08):** «Номер» — опциональная примета, не обязательный госномер: DB `Car.plate String?` (миграция `20260908111246_make_car_plate_optional`; убран дрейфующий `User_deletedAt_idx` вне схемы), DTO `optional()` + `""` → `null`, сериализатор опускает null-ключ (fail-closed контракт), создание поездки требует только сам автомобиль. Mini-app: плейсхолдер «Например: 583», subtitle профиля без номера. Tests: `car-optional-plate.test.ts` (+6).

- **UI consistency fixes (2026-09-08):** модальный сабмит-стандарт — sticky-футер Box+Separator+size="l" stretched primary (ReportModal приведён, 7/7 форм); Banner actions всегда `size="m"` явно (VKUI-дефолт size="s", не "l"); степперы мест `size="s"` явно. Card mode="outline" без ручных borderRadius/backgroundColor (токены). Цена в RouteLine — `Title level="3"` (токены), Spacing только 4-кратные (4/8/12/16/24). FormItem `bottom` — строкой/Caption, цвет ошибки из `.statusError .bottom` (не кастомный span). AuthGate — AppPanelHeader. `aria-label="Назад"` на PanelHeaderBack не нужен (VKUI label → VisuallyHidden). Иконка-сердце 24→30 в CreateReviewModal — осознанный ресайз под тап-зону 46px, единственный в приложении.

- **VKUI audit fixes (2026-09-08):** `transformVKBridgeAdaptivity` — официальный хелпер из vkui.io (getViewWidthByViewportWidth/getViewHeightByViewportHeight, enum ViewWidth, density). Behavior change: adaptive viewport по брейкпоинтам VKUI 320/768/1024/1280 (320–767 = MOBILE, 768–1023 = SMALL_TABLET, 1024–1279 = TABLET, ≥1280 = DESKTOP) + viewHeight 415/720 (раньше не передавался); force_mobile_compact → density "compact", force_mobile → "regular". HomePanel Search: aria-label → slotProps.input + input tabIndex=-1 (v8-паттерн, a11y). ModalRoot в SSR-тестах модалок легитимен — это внутренний рендерер useModalManager (ContextHolder), обоснование в комментариях тестов. VKUI-использование аудитено против v8-доков: устаревших пропсов в рантайме нет (getRef/accessible/FixedLayout/Div/afterCaption отсутствуют; Alert onClose(reason)+onClosed оба валидны в 8.4).
...
- **ConfirmProvider queue (2026-09-07):** profile deletion silently did nothing — `confirm()` promise hung on Alert close-animation end (`action` deferred to `onExited`/animationend, never fired in some environments; backend logs showed zero `DELETE /users/me`). Provider now serializes via `chainConfirmTask` (`helpers/confirmQueue.ts`) and resolves synchronously on tap with instant unmount (no exit animation — reliability first); overlay/Esc/X resolve `false` via `onClose` (except `click-item`), `onClosed` is a safety net. Single confirms unchanged. Verified in-browser (Playwright): both dialogs → `DELETE` fires. Tests: `confirmQueue.test.ts` (3).
- **Trip secondary actions inline (2026-09-07):** Share/Report live in a «Дополнительно» section at the bottom of `TripDetailsPanel` (new `TripSecondaryActions`, `ButtonGroup vertical` + 2× secondary buttons). Header kebab (`TripActionsSheet`) removed — VK client system buttons occupy the top-right corner and overlapped it. Handlers unchanged. Report button gated to passengers with a booking (`!isOwnTrip && myBooking`; backend `canReport` trip branch dropped the driver — own-trip reports get `403 FORBIDDEN`) — strangers got `403 FORBIDDEN` from `canReport`; `ReportModal` maps 403 to «Жалоба недоступна». Entry button reflects sent state (`alreadyReported` from `GET /reports`): «Жалоба уже отправлена», disabled — modal never opens. Tests: `TripActionsMenu.test.tsx` (6 SSR), `reports.test.ts` (+1 own-trip 403).

- **Prod переехал на этот хост + dev-лимиты ×100 + tgui-добор (2026-09-10):** диагноз «Ошибка авторизации» на iPhone: `tg-edem.binetc.fun` отдавал vite-dev, а бэкенд `.71` работал в dev-bypass без `TELEGRAM_BOT_TOKEN` (доказано: `hash=dev-hash` получил 200 с JWT). Прод поднят здесь: `docker compose up -d db backend` (:3000, `BACKEND_BIND_ADDR=0.0.0.0`), `prisma migrate deploy` в CMD, справочник городов — новый `backend/prisma/seed-cities.ts` (prod-safe, только недостающие; `SEED_CITIES`/`normalizeCityName` вынесены в `cities-data.ts`, seed.ts импортирует — поведение dev-сида не менялось). В root `.env` добавлен `CORS_ORIGINS` (требует compose). Проверено: Host `tg-edem.binetc.fun` → TG prod-dist (без vite-маркеров), dev-hash → 401, токен `.env` = `@edem_mini_bot` (getMe). Прод-БД переиспользовала том `vk-mini-edem_pgdata` (2 юзера от 08–09.09). Dev остался на :3011+:3012. Осталось на стороне инфры: переключить reverse proxy `tg-edem.binetc.fun` → `.67:3000`, добавить `tg-dev.binetc.fun` → `.67:3012`, остановить vite-dev на `.71`. Внимание: root `.env` держит `TG_AUTH_RATE_MAX=100000` (ослаблен) — для строгого прода убрать строку (дефолт 5). Env: `devRateMax()` ×100 всех MAX-лимитов только при `NODE_ENV=development` (тесты `test` и прод строгие; `user-rate-limit.test.ts` зелёный). Tgui-добор: `Timeline` (адреса) + `Blockquote` (комментарий водителя) в TripDetails, `VisuallyHidden` в Onboarding (`.sr-only` удалён); осознанно отказано: ButtonCell (нет subtitle), IconContainer (без размеров), Card (пример — custom divs), широкая Типографика (шкала не 1:1), Switch/Slider (ломают e2e-fill), Rating (неконтролируемый), Modal (убьёт диплинки), Skeleton (ассёрты загрузки).
