...
export const PassengerTripCard: FC<{
  ...
  // «Написать» водителю — с момента брони (pending) и после подтверждения,
  // пока поездка не отменена и не уехала.
  const canWriteDriver =
    booking.trip.driver.vkUserId != null &&
    (booking.status === "pending" || booking.status === "confirmed") &&
    booking.trip.status !== "cancelled" &&
    !isPast(booking.trip.departureAt);
  const hasDetails = Boolean(
    booking.comment || canReview || booking.hasReview || canWriteDriver
  );

  const handleWriteDriver = (event: { stopPropagation: () => void }) => {
    // Клик по кнопке не должен открывать детали поездки (onOpen карточки).
    event.stopPropagation();
    if (booking.trip.driver.vkUserId != null) {
      void openVkMessages(booking.trip.driver.vkUserId);
    }
  };
      ...
      {hasDetails && (
        <>
          ...
          {canWriteDriver && (
            <>
              <Spacing size={12} />
              <Button size="s" mode="secondary" onClick={handleWriteDriver}>
                Написать в VK
              </Button>
            </>
          )}
