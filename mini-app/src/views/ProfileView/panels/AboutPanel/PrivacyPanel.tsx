...
export const PrivacyPanel: FC<PrivacyPanelProps> = ({ id, onBack }) => {
  return (
    <Panel id={id}>
      ...
      <Group header={<Header size="s">1. Какие данные мы используем</Header>}>
        <Box>
          <Paragraph>
            ...
            пользователь указывает самостоятельно: сведения об автомобиле,
            отзывы, комментарии и настройки приложения.
          </Paragraph>
        </Box>
      </Group>
      ...
      <Group
        header={<Header size="s">3. Что видят другие пользователи</Header>}
      >
        <Box>
          <Paragraph>
            Другие пользователи могут видеть имя, фотографию, рейтинг, отзывы,
            модель и цвет автомобиля. Госномер не публикуется в открытом профиле
            и поиске. Точные адреса сейчас не отображаются другим пользователям.
          </Paragraph>
        </Box>
      </Group>
      ...
      <Group header={<Header size="s">4. Хранение и защита</Header>}>
        <Box>
          <Paragraph>
            ...
            данных пользователей и не передаем данные третьим лицам, за
      ...
      <Group header={<Header size="s">5. Права пользователя</Header>}>
        <Box>
          <Paragraph>
            ...
            своих данных, обратившись в поддержку сервиса.
          </Paragraph>
        </Box>
      </Group>
