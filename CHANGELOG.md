...
- Удаление профиля молча не работало: промис `ConfirmProvider.confirm` висел на событии завершения анимации закрытия (`AlertBase` откладывает action до `onExited`/*animationend*), а в части окружений это событие не приходит — второй диалог не открывался, запрос не уходил (в логах бэкенда — ноль `DELETE /users/me`). Провайдер переписан: очередь через `chainConfirmTask` (`helpers/confirmQueue.ts`) + синхронный резолв в тапе с мгновенным анмаунтом (exit-анимации нет — надёжность важнее); оверлей/Esc/крестик резолвят `false` через `onClose` (кроме `click-item`), `onClosed` — safety net. Одиночные confirm'ы (брони, поездки) ведут себя как раньше. Проверено в браузере (Playwright): оба диалога → `DELETE` уходит. Tests: `confirmQueue.test.ts` (3: порядок, блокировка второго, живучесть цепочки).
