...
- Кнопка «Пожаловаться на поездку» видна только участникам поездки (водитель или пассажир с активной бронью): `POST /reports` иначе отвечал `403 FORBIDDEN` (`canReport`, связь водитель/пассажир обязательна). Посторонним доступна только «Поделиться». `ReportModal` показывает на 403 понятный текст «Жалоба недоступна — жалобы доступны участникам поездки» вместо generic-ошибки.
